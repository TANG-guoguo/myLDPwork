...
def ahead_tree_update(ahead_tree, tree_height, theta, branch, translation_vector, layer_index):
    for node in ahead_tree.leaves():
        if not node.data.divide_flag:     #当前节点不分裂
            continue

        elif (tree_height > 0 and node.data.divide_flag) and (node.data.frequency < theta):    #当前节点频率小于阈值，不分裂
            node.data.divide_flag = False
            continue

        else:     #当前节点频率大于阈值，需要分裂
            TempItem0 = np.zeros(node.data.interval.shape)   #TempItem0用于计算分裂成的两个节点的区间范围起始值（a,b）
            for j in range(0, len(node.data.interval), 2):    #其实j只等于0

                if node.data.interval[j + 1] - node.data.interval[j] > 1:    #当前节点粒度>2，还可以二分
            ...
            for item1 in translation_vector:    #translation_vector记录该层划分粒度？
...
def main_func(repeat_time, domain_size, branch, ahead_tree_height, theta, real_frequency, query_interval_table, epsilon, data_path, data_name, data_size_name, domain_name):
    ...
    while repeat < repeat_time:
        ...
        #print("用户分组：",user_dataset_partition)
