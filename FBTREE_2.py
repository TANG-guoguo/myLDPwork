...
class Nodex(object):
    def __init__(self, frequency, ffrequency,divide_flag, count, interval):
        ...
        self.ffrequency = ffrequency
        self.divide_flag = divide_flag
        self.count = count
        self.interval = interval

def theta_calculation(user_scale_in_each_layer, epsilon, branch):
...
def AHEAD_Mean_Consistency(FBTree):
    for node in FBTree.expand_tree(mode=Tree.WIDTH, sorting=False):
        vname = node
        if vname =='Root':
            continue
            ...
            fv = FBTree[vname].data.frequency
            vpname = FBTree.parent(vname).tag    #当前节点v的父节点的名字
            #vpB = FBTree[vpname].data.count   #父节点的分支数
            vpB = len(FBTree.children(vpname))  # 父节点的分支数
            fpv = FBTree[vpname].data.frequency
            fvsum = fv
            for u in FBTree.siblings(vname):
                fvsum += u.data.frequency
...
def AHEAD_weighted_averaging(FBTree,layer,user_scale_in_each_layer):   #对全树节点进行自下而上加权平均处理

    NOWlayer=layer+1
    while(NOWlayer<=0):
        node_num=FBTree.size(tmp_h+1+abs(NOWlayer))
        varience_low = 4 * math.exp(epsilon) / (user_scale_in_each_layer * (math.exp(epsilon) - 1) ** 2)  # 下层方差
        varience_this = 4 * math.exp(epsilon) / (user_scale_in_each_layer * (math.exp(epsilon) - 1) ** 2)  # 本层方差
        for i in range(0,node_num):
            name = 'L-' + str(NOWlayer) + 'N-' + str(i)
            fv = FBTree[name].data.frequency
            n = len(FBTree.children(name))  # n为该节点所包含的子节点个数
            varience_child = n * varience_low  # 子节点方差
            lambda1 = varience_child / (varience_this + varience_child)
            lambda2 = 1 - lambda1
            fv_child=0
            for u in FBTree.children(name):
                fv_child+=u.data.frequency
            newfv = lambda1 * fv_child + lambda2 * fv
            FBTree[name].data.frequency = newfv  # 更新树中的节点频率
            FBTree[name].data.ffrequency = newfv  # 更新树中的节点频率
...
def AHEAD(FBTree,theta,epsilon,datalist,start_size,d):
    ...
    layertag=-1
    tmpflist=[]
    for i in range(0,d//nowsize):
        name = 'L-' + str(layertag+1) + 'N-' + str(i)
        tmp=FBTree[name].data.ffrequency
        tmpflist.append(tmp)
    print(tmpflist)
    AHEAD_CUT0 = [[j, j + start_size - 1] for j in range(0, d, start_size)]
    DVDTAG0=[1 for j in range(0, d, start_size)]
    assert len(tmpflist) == len(AHEAD_CUT0)

    FLAG = 0
    while(True):
        NEED=0
        for i in range(0,len(DVDTAG0)):
            if max(tmpflist) < theta:
                break
            else:
                if DVDTAG0[i]==1 and tmpflist[i]>theta:
                    NEED=1
                    break
        ...
        if NEED==1:  # 有需要划分的节点
            AHEAD_CUT = []
            DVDTAG=[]
            for i in range(0, len(tmpflist)):
                if tmpflist[i] > theta and DVDTAG0[i]==1:
                    AHEAD_CUT.append([AHEAD_CUT0[i][0], AHEAD_CUT0[i][0] + nowsize // 2 - 1])
                    AHEAD_CUT.append([AHEAD_CUT0[i][0] + nowsize // 2, AHEAD_CUT0[i][1]])
                    DVDTAG.append(1)
                    DVDTAG.append(1)
                else:
                    AHEAD_CUT.append(AHEAD_CUT0[i])
                    DVDTAG.append(0)   #这里不划分以后也不允许再划分
            print("AHEAD_CUT", AHEAD_CUT)
            print("DVDTAG",DVDTAG)
            # 按cut收集用户回答得到新的tmpflist
            ...
            user_num_thislayer = AHEADUSERNUM // downlayer
            pdatalist = get_user_pdata(datalist, user_num, INTERVAL, node_num, epsilon, user_num_thislayer)
            # 聚合频率
            ...
            # 根据NNFV建树
            k = 0
            for i in range(0, len(AHEAD_CUT0)):  # 遍历节点、二分
                fathername = 'L-' + str(layertag + 1) + 'N-' + str(i)
                if (FBTree[fathername].data.ffrequency > theta and FBTree[fathername].data.divide_flag==True):  # 大于阈值，二分
                    num_in_node = FBTree[fathername].data.count // 2
                    Lname = 'L-' + str(layertag) + 'N-' + str(k)
                    FBTree.create_node(tag=Lname, identifier=Lname, parent=fathername,
                                       data=Nodex(NNFV[k], NNFV[k], True, num_in_node, np.array(AHEAD_CUT[k])))
                    k += 1
                    Rname = 'L-' + str(layertag) + 'N-' + str(k)
                    FBTree.create_node(tag=Rname, identifier=Rname, parent=fathername,
                                       data=Nodex(NNFV[k], NNFV[k], True, num_in_node, np.array(AHEAD_CUT[k])))
                    k += 1
                else:
                    FBTree[fathername].data.divide_flag = False
                    Lname = 'L-' + str(layertag) + 'N-' + str(k)
                    num_in_node = FBTree[fathername].data.count
                    FBTree.create_node(tag=Lname, identifier=Lname, parent=fathername,
                                       data=Nodex(NNFV[k], NNFV[k], False, num_in_node, np.array(AHEAD_CUT[k])))
                    k += 1
            assert k == len(AHEAD_CUT)
            FBTree.show(key=False)
            #为下一层划分做准备
            nowsize = nowsize//2
            layertag -=1
            tmpflist = NNFV
            AHEAD_CUT0 = AHEAD_CUT
            DVDTAG0=DVDTAG


        else:
            restusernum = len(datalist)
            if restusernum>0:  #有剩余
                print("剩余",restusernum)  # 直接均分tmpflist的频率#若有剩余用户则不要了！!!!
            #加权平均处理
            AHEAD_weighted_averaging(FBTree, layertag+1, AHEADUSERNUM//downlayer)
            #删除父节点划分TAG为0的节点
            KILLLIST=[]
            for node in FBTree.expand_tree(mode=Tree.WIDTH, sorting=False):
                vname = node
                if vname == 'Root':
                    ...
                    vpname = FBTree.parent(vname).tag  # 当前节点v的父节点的名字
                    if FBTree[vpname].data.divide_flag==False:
                        KILLLIST.append(vname)
            for name in KILLLIST[::-1]:
                FBTree.remove_node(name)
            FBTree.show(key=False)
            print("hh")


            #一致化处理
            AHEAD_Mean_Consistency(FBTree)


            #均分频率
            result=[]
            for node in FBTree.expand_tree(mode=Tree.DEPTH, sorting=False):
                if(len(FBTree.children(node))==0):
                    fv = FBTree[node].data.ffrequency
                    n = FBTree[node].data.count
                    for j in range(0, n):
                        result.append(fv / n)
            return result
...
def main_func(datalist, user_num, d, epsilon, start_size):
    ...
    if CUT[-1][-1]!=d-1:
        last = CUT[-1][-1]
        CUT.append([last+1,d-1])
    node_num = len(CUT) #初次划分节点个数
    ...
    while(True):
        ...
        print("剩余用户数量：",user_num)
        if(FLAG==1):
            user_num_thislayer = user_num-AHEADUSERNUM
        else:
            user_num_thislayer = (user_num-AHEADUSERNUM)// (tmp_h-layer+1)
    ...
    theta = theta_calculation(AHEADUSERNUM//downlayer,epsilon,2)
    print("theta=",theta)
    result=AHEAD(FBTree,theta,epsilon,datalist,start_size,d)
    LOWLOWEST_NODE_FV=result
    ...
    print("最终频率=",LOWLOWEST_NODE_FV)
    print(len(LOWLOWEST_NODE_FV),"=1024")
    print("结束时剩余用户",len(datalist))
    #LOWEST_NODE_FV=non_negativity(LOWEST_NODE_FV, d)
    #print("非负处理后最终频率=", LOWEST_NODE_FV)
    return LOWLOWEST_NODE_FV
...
if __name__ == "__main__":
    ...
    for i in range(0,repeat):
        ...
        datalist = []
        for DT in dataset:
            datalist.append(DT - 1)  ##############dataset1是i-1
    print("均方误差", MSELIST)
